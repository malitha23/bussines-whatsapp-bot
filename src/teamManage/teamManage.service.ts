...
export class TeamManageService {
  ...
  private async getBusinessByUser(
    email: string,
  ): Promise<{ businessId: number; businessName: string; business: Business }> {
    ...
    const usersWithPermissions = await Promise.all(
      users.map(async (user) => {
        ...
        return {
          ...
          permissions,
          custom_permissions:
            'custom_permissions' in user ? user.custom_permissions : [],
        };
      }),
    );
  ...
  async createUser(email: string, createUserDto: CreateUserDto) {
    const { businessId, business, businessName } =
      await this.getBusinessByUser(email);
    ...
    const role = await this.roleRepo.findOne({
      where: { name: createUserDto.role_type },
    });
    if (!role)
      throw new NotFoundException(`Role ${createUserDto.role_type} not found`);

    // Fetch existing role permissions for this role + business
    let rolePermissions = await this.rolePermissionRepo.find({
      where: { role: { id: role.id }, business: { id: businessId } },
      relations: ['permission'],
    });

    // Default permissions for each role
    const defaultPermissionNames: string[] =
      createUserDto.role_type === UserRole.MANAGER
        ? [
            'view_dashboard',
            'manage_team',
            'manage_roles',
        ...
        : ['view_dashboard', 'view_orders', 'quick_view_orders'];

    // Fetch permission records from permissions table
    const defaultPermissions = await this.permissionRepo.find({
      where: defaultPermissionNames.map((name) => ({ name })),
    });

    // Insert only missing permissions
    const missingPermissions = defaultPermissions.filter(
      (dp) => !rolePermissions.some((rp) => rp.permission?.id === dp.id),
    );

    if (missingPermissions.length > 0) {
      const newRolePermissions = missingPermissions.map((permission) =>
        this.rolePermissionRepo.create({
          role: { id: role.id }, // pass object with id
          permission: { id: permission.id },
          business: { id: businessId }, // NOT businessId directly
          status: 1,
        }),
      );
    ...
    // Map final permissions for return
    const permissions = rolePermissions
      .map((rp) => rp.permission?.name)
      .filter(Boolean) as string[];
  ...
  async updateUser(
    ...
    if (!updatedBy)
      throw new NotFoundException('User performing update not found');
  ...
  async bulkDeleteUsers(email: string, bulkDeleteDto: BulkDeleteDto) {
    ...
    return {
      message: `${bulkDeleteDto.userIds.length} users removed successfully`,
    };
  }
  ...
  async getRoleDefinitions(email: string) {
    ...
    const roleDefinitions: Record<
      string,
      {
        name: string;
        description?: string;
        permissions: { name: string; status: number }[];
      }
    > = {};
  ...
  async updateRolePermission(
    ...
    const action =
      status === null ? 'removed' : status === 1 ? 'enabled' : 'disabled';
