...
export class CreateInventoryTables1762110667043 implements MigrationInterface {
  name = 'CreateInventoryTables1762110667043';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE \`product_variants\` (\`id\` int NOT NULL AUTO_INCREMENT, \`variant_name\` varchar(255) NOT NULL, \`price\` decimal(10,2) NOT NULL, \`stock\` int NOT NULL DEFAULT '0', \`sku\` varchar(255) NULL, \`is_active\` tinyint NOT NULL DEFAULT 1, \`productId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `CREATE TABLE \`product_images\` (\`id\` int NOT NULL AUTO_INCREMENT, \`image_url\` varchar(255) NOT NULL, \`is_main\` tinyint NOT NULL DEFAULT 0, \`productId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `CREATE TABLE \`inventory_transactions\` (\`id\` int NOT NULL AUTO_INCREMENT, \`quantity\` int NOT NULL, \`type\` enum ('IN', 'OUT') NOT NULL, \`note\` varchar(255) NULL, \`created_at\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, \`productId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `CREATE TABLE \`products\` (\`id\` int NOT NULL AUTO_INCREMENT, \`name\` varchar(255) NOT NULL, \`base_price\` decimal(10,2) NOT NULL, \`description\` varchar(255) NULL, \`is_active\` tinyint NOT NULL DEFAULT 1, \`created_at\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, \`businessId\` int NULL, \`subsubCategoryId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `CREATE TABLE \`inventory_stock\` (\`id\` int NOT NULL AUTO_INCREMENT, \`quantity\` int NOT NULL, \`location\` varchar(255) NOT NULL DEFAULT 'warehouse', \`updated_at\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, \`variantId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE \`user_states\` CHANGE \`name\` \`name\` varchar(100) NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE \`user_states\` CHANGE \`state\` \`state\` varchar(255) NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE \`user_states\` CHANGE \`last_message\` \`last_message\` text NULL`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE \`inventory_stock\` ADD CONSTRAINT \`FK_7daff23230423fa09bb00a640ff\` FOREIGN KEY (\`variantId\`) REFERENCES \`product_variants\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE \`inventory_stock\` DROP FOREIGN KEY \`FK_7daff23230423fa09bb00a640ff\``,
    );
    await queryRunner.query(
      `ALTER TABLE \`orders\` DROP FOREIGN KEY \`FK_e5de51ca888d8b1f5ac25799dd1\``,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE \`inventory_transactions\` DROP FOREIGN KEY \`FK_f21250669a9728997c6d8f6f5da\``,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE \`user_states\` CHANGE \`last_message\` \`last_message\` text NULL DEFAULT 'NULL'`,
    );
    await queryRunner.query(
      `ALTER TABLE \`user_states\` CHANGE \`state\` \`state\` varchar(255) NULL DEFAULT 'NULL'`,
    );
    await queryRunner.query(
      `ALTER TABLE \`user_states\` CHANGE \`name\` \`name\` varchar(100) NULL DEFAULT 'NULL'`,
    );
    await queryRunner.query(
