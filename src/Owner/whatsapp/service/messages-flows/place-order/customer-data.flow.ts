...
export async function handleCustomerDetails(
  ...
  if (
    text === '0' &&
    !['collect_customer_email', 'collect_customer_phone'].includes(state)
  ) {
    switch (state) {
      case 'collect_customer_name': {
        ...
        if (selectedVariant?.product) {
          ...
          await sendManager.sendMessage({
            phone,
            text: `⚠️ Could not load previous variant.`,
          });
        }
        return;
      }
      ...
      case 'collect_customer_address':
        ...
        await sendManager.sendMessage({
          phone,
          text: await getBotMessage(
            botMessageRepo,
            businessId,
            language,
            'customer_data_back_to_name',
          ),
        });
        return;
    }
  ...
  switch (state) {
    ...
    case 'collect_customer_phone':
      stateData.customerPhone = text === '0' ? phone.split('@')[0] : text;
      ...
      ============================================================ */
      const title = await getBotMessage(
        botMessageRepo,
        businessId,
        language,
        'customer_data_confirm_details_title',
      );
      const labelName = await getBotMessage(
        botMessageRepo,
        businessId,
        language,
        'customer_data_label_name',
      );
      const labelAddress = await getBotMessage(
        botMessageRepo,
        businessId,
        language,
        'customer_data_label_address',
      );
      const labelPhone = await getBotMessage(
        botMessageRepo,
        businessId,
        language,
        'customer_data_label_phone',
      );
      const labelEmail = await getBotMessage(
        botMessageRepo,
        businessId,
        language,
        'customer_data_label_email',
      );
      const confirmInstruction = await getBotMessage(
        botMessageRepo,
        businessId,
        language,
        'customer_data_confirm_instruction',
      );

      const changeName = await getBotMessage(
        botMessageRepo,
        businessId,
        language,
        'customer_data_change_name',
      );
      const changeAddress = await getBotMessage(
        botMessageRepo,
        businessId,
        language,
        'customer_data_change_address',
      );
      const changeEmail = await getBotMessage(
        botMessageRepo,
        businessId,
        language,
        'customer_data_change_email',
      );
      const changePhone = await getBotMessage(
        botMessageRepo,
        businessId,
        language,
        'customer_data_change_phone',
      );
      ...
      const finalMessage =
        ...
        (stateData.customerEmail
          ? `${labelEmail} ${stateData.customerEmail}\n`
          : '') +
        `\n${confirmInstruction}\n` +
        `${changeName}\n` +
        `${changeAddress}\n` +
      ...
      await sendManager.sendMessage({ phone, text: finalMessage });
      return;
  }
}
