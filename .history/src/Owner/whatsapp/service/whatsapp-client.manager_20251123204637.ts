...
export class WhatsAppClientManager {
  ...
  private qrSentMap = new Map<number, boolean>();
  ...
  async createClient(businessId: number): Promise<Client> {
    ...
    // Track QR send per business
    if (!this.qrSentMap) this.qrSentMap = new Map<number, boolean>();

    client.on('qr', (qr) => {
      if (this.qrSentMap.get(businessId)) return; // already sent
      this.qrSentMap.set(businessId, true);

      this.logger.log(`📱 QR scan for Business ${businessId}`);
    ...
    client.on('ready', () => {
      ...
      if (this.gateway) this.gateway.sendReady(businessId);
    });
    ...
    client.on('disconnected', () => {
      ...
      this.qrSentMap.delete(businessId); // reset QR flag for reconnection
      if (this.gateway) this.gateway.sendDisconnected(businessId);
    });
