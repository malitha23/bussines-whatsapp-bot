...
async createClient(businessId: number): Promise<{
  ...
  // If already exists and already connected → don't destroy, just return it
  if (this.clients.has(businessId)) {
    const existing = this.clients.get(businessId);

    if (existing?.info?.me) {
      this.logger.log(`⚡ Client already active for Business ${businessId}`);
      return {
        status: 'success',
    ...
    // If exists but not connected → DO NOT stopClient()
    this.logger.warn(`♻️ Client exists but not connected. Reinitializing safely...`);
  }
