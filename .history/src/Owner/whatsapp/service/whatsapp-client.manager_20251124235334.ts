...
export class WhatsAppClientManager {
  ...
  async createClient(businessId: number): Promise<{
    ...
    // Stop existing client to avoid EBUSY errors
    ...
    client.on('ready', async () => {
      try {
        ...
        this.logger.error(`Error updating session status for business ${businessId}: ${err}`);
      }
    });
    ...
    client.on('disconnected', async () => {
      try {
        ...
        this.logger.error(`Error handling disconnect for business ${businessId}: ${err}`);
      }
    });

    // Initialize client with retry for Windows EBUSY
    let retries = 3;
    for (let i = 0; i < retries; i++) {
      try {
        ...
        if (err.code === 'EBUSY' && i < retries - 1) {
          this.logger.warn(`EBUSY error initializing client, retrying...`);
