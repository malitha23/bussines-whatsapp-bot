...
export class WhatsAppClientManager {
  ...
  // Prevent reconnect if mobile logout happened
  if (this.mobileLogout?.has(businessId)) {
    this.logger.warn(`⛔ Business ${businessId} logged out from mobile. Manual reconnect required.`);
    return {
      status: 'error',
      message: 'Mobile logout detected. Please reconnect manually.',
      connected: false,
      client: null,
    };
  }

  // If client already exists and connected
  ...
  client.on('disconnected', async (reason: WAState | 'LOGOUT') => {
    try {
      ...
      // Destroy Puppeteer session safely
      ...
      if (reason === 'LOGOUT') {
        ...
        if (!this.mobileLogout) this.mobileLogout = new Set<number>();
