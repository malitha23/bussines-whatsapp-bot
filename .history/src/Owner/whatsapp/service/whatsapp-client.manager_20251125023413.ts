...
//     // Safe destroy: check if client is still usable
//     if (client && !client.destroy) {
//       for (let i = 0; i < 3; i++) {
//         try {
//           await client.destroy();
//           this.logger.log(`Client destroyed safely for Business ${businessId}`);
//           break;
//         } catch (err: any) {
//           if (err.code === 'EBUSY' && i < 2) {
//             this.logger.warn(`EBUSY destroying client, retry ${i + 1}`);
...
//     // Delete session folder only after destroy
//     if (['NAVIGATION', 'LOGOUT'].includes(reason)) {
//       const sessionPath = this.getSessionPath(businessId);
...
//         } catch (err) {
//           this.logger.error(`Failed to delete session folder: ${err}`);
//         }
//       }
//     }
...
//   } catch (err) {
//     this.logger.error(`Error handling disconnected for Business ${businessId}: ${err}`);
//   }
// });

client.on('disconnected', async (reason) => {
  console.log('WhatsApp client was logged out:', reason);
  this.handleClientReinitialization(isReinitializing, client); // Reset and reinitialize on disconnection
});
  ...
  async handleClientReinitialization (isReinitializing: boolean, client: Client) {
  if (!isReinitializing) {
    isReinitializing = true;
    try {
      await this.resetClient(client); // Reset and reinitialize the client
