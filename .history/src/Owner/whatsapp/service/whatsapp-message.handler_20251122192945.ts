...
export class WhatsAppMessageHandler {
  ...
  async handleIncomingMessage(
    ...
    const upload_payment_receipt = new Set([
    ...
    if (cleanText === '0') {
      ...
      // If inside upload payment receipt submenu → go back to main menu
      if (upload_payment_receipt.has(state)) {
        const prevState = userState?.previous_state || 'main_menu';
        await this.restorePreviousState(client, businessId, phone, name, business, prevState);
        return;
      }
    ...
    switch (state) {
      ...
      case 'select_receipt_option':
        await handleUploadPaymentReceipt(client, phone, businessId, name, cleanText, language, this.saveUserState.bind(this), this.sendMainMenu.bind(this), this.orderRepo);
        return;
