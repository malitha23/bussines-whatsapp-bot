...
export class WhatsAppMessageHandler {
  ...
  async handleIncomingMessage(
    ...
    const placeOrderStates = new Set([
      'place_order', 'category_selection', 'subcategory_selection',
    ...
    const myOrdersSubMenus = new Set([
      ...
      'shipped_orders', 'delivered_orders', 'canceled_orders', 'refunded_orders'
    ]);

console.log('dddddddddddddddddddddddddddddddddddddddddddddddddd' + state);
    if (cleanText === '0') {
    ...
    switch (state) {
      ...
      case 'select_receipt_option':
        ...
        // Save state: now in select_order_for_receipt_upload
        await this.saveUserState(
          ...
          language,
          'select_receipt_option' // pressing 0 goes back here
        );

        break;

      case 'select_order_for_receipt_upload':
        console.log('sdsadfsds cleanText' + cleanText);
        if (cleanText === '0') {
          // Back කරන්න select_receipt_option වෙත
          await client.sendMessage(phone, "↩️ Back to Upload Payment Receipt options.");

          await this.saveUserState(businessId, phone, name, {}, 'select_receipt_option', language, 'main_menu');
